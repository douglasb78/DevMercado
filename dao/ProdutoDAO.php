...
class ProdutoDAO {
    ...
    public function contarBusca(string $termo): int {
        ...
        $stmt = $this->pdo->prepare(
            'SELECT COUNT(*) FROM produtos
              WHERE (id::text ILIKE :termo
                  OR nome ILIKE :termo
                  OR descricao ILIKE :termo2)
                AND is_deleted = false'
        );
    ...
    public function contarBuscaAdmin(string $termo): int {
        $like = '%' . trim($termo) . '%';
        $stmt = $this->pdo->prepare(
            'SELECT COUNT(*)
               FROM produtos p
    ...
    public function buscarPorId(int $id, bool $incluirRemovidos = false): ?Produto {
        $sql = 'SELECT p.*, u.nome AS fornecedor_nome
                  FROM produtos p
                  JOIN usuarios u ON u.id = p.fornecedor_id
                 WHERE p.id = :id';
        if (!$incluirRemovidos) {
